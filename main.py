...
P_HOW_ARE_YOU = [
    ...
    r"\bти\s+норм\b",
    r"\bшо\s+ти\b",         # <-- додано: ловить "нері шо ти"
    r"\bщо\s+ти\b",         # <-- додано: ловить "нері що ти"
]
P_WHAT_DOING_NOW = [
    r"\bшо\s+робиш\b",
    r"\bщо\s+робиш\b",
...
# ===== New “smart” fallbacks (без "перефразуй") =====
FALLBACKS = [
    "Я трошки загубився в листочках 🌿 Але я з тобою. Дай 1–2 ключові слова — і я підхоплю.",
    "Ой 😼 Я не впіймав сенс. Обери, що ти мав на увазі: 1) поради 2) думку 3) інфу? 👀",
    "Я не маю точних даних про це 🌿 Але можу: (1) пояснити загально, (2) придумати ідеї, (3) допомогти сформулювати. Що обираєш?",
    "Я зараз не знаю точну відповідь 😅🌿 Зате скажи: це про людей, про софт, чи про нашу команду?",
    "Я завис, як кіт на підвіконні 😼🍃 Дай одну підказку: тема/слово/контекст — і я відповім нормально.",
]

def smart_fallback(raw_text: str, q: str) -> str:
    has_qmark = "?" in (raw_text or "")
    longish = len(q) > 60

    choose = [
        "Я не впевнений, що правильно зловив сенс 🌿 Обери: 1) поясни простими словами 2) накидай ідей 3) допоможу сформулювати 👀",
        "Я можу не вгадати зараз 😼🌿 Ти хочеш: 1) відповідь по факту 2) пораду 3) креативний варіант? ✨",
        "Я не маю точних даних 🌱 Але можу бути корисним: 1) коротко 2) детально 3) списком кроків. Що краще?",
    ]

    key = [
        "Я трошки загубився 🌿 Кинь 1–2 ключові слова або контекст — і я підхоплю.",
        "Не дійшло, але я тут 😼🍃 Скажи «це про ___» — і буде нормальна відповідь.",
        "Я не зловив тему 🌱 Одне слово-підказка — і я вже в строю ✨",
        "Я завис 😅🌿 Дай приклад або уточни одним реченням, що саме треба.",
    ]

    out = random.choice(choose if (has_qmark or longish) else key)
    return neri_style(out)
...
async def telegram_webhook(request: Request):
    ...
    if text == "/start":
        ...
        # погода
            ...
            # 1.5) smalltalk ПЕРЕД "про себе" (фікс "нері шо ти")
            else:
                        ...
                        # 8) старі інтенти (резерв)
                            ...
                            if found:
                                ...
                                if fact:
                                    ...
                                    # 10) розумний fallback
                                    reply = smart_fallback(raw_text, q)
