...
# ===== Personality spice (екстраверт, але не перебор) =====
CAPS_RATE = 0.08   # ~8% повідомлень з легким КАПСОМ (рідко)
HYPE_RATE = 0.22   # ~22% додасть короткий вигук/теплий вступ (помірно)

HYPE_PREFIXES = [
    "ОЙ!", "О!", "ЕЙ!", "ХЕЙ!", "ЙО!", "УУУ!", "ААА, ПРИВІТ!", "ОПА!", "ОГО!",
]
SOFT_TAGS = [
    "я тут 💚", "я поруч 🌿", "давай спокійно 🍃", "чайку? 🍵", "музичку? 🎶"
]

def maybe_caps(s: str) -> str:
    if random.random() > CAPS_RATE:
        return s
    parts = s.split(" ", 6)
    if len(parts) <= 3:
        return s.upper()
    head = " ".join(parts[:3]).upper()
    tail = " ".join(parts[3:])
    return f"{head} {tail}"

def maybe_hype_prefix(s: str) -> str:
    if random.random() > HYPE_RATE:
        return s
    pref = random.choice(HYPE_PREFIXES)
    tag = random.choice(SOFT_TAGS)
    return f"{pref} {s} ({tag})"

def style_reply(s: str) -> str:
    low = (s or "").lower()
    # якщо це підтримка/сум/тривога — без вигуків і капсу
    if any(x in low for x in ["трив", "страш", "сум", "погано", "самот", "втом", "боляче", "важко"]):
        return s
    s = maybe_hype_prefix(s)
    s = maybe_caps(s)
    return s
...
ABOUT_REPLIES = [
    "Я Нері — маскот і символ команди 💚🌿 Ніжний, турботливий і ДУЖЕ товариський (але без шуму) 🍵✨",
    "Я тут, щоб тримати атмосферу 🌿 і щоб у чаті було тепло. Хочеш — познайомимось ближче? 👀",
    "Я Нері: талісман, вайб і трохи балакун 😼🌱 Люблю природу, музику і зелений чай 🍵🎶",
    "Я той, хто питає «як ти?» і реально слухає 🌿💚 То як ти зараз?",
]

INTERESTING_REPLIES = [
    "Міні-факт 🌿: інколи найкращий відпочинок — 10 хвилин музики й чай 🍵🎧 А що ти слухаєш останнім часом?",
    "Природа дуже лікує голову 🌳🍃 Ти більше по лісу/парку чи по нічному місту?",
    "Я люблю живі чати, але без тиску 🌿✨ Розкажи 1 хорошу дрібницю з твого дня 👀",
    "Маленькі кроки теж кроки 🍃 А що ти сьогодні зробив такого, що було не дарма?",
    "Якщо хочеш, я можу задати тобі 3 питання «на побалакати» 😼🌿",
]
...
FALLBACKS = [
    "Я ніби зрозумів, але не на 100% 😅🌿 Скажи інакше — і я підхоплю 👀✨",
    "Окей, давай уточнимо: ти питаєш про команду, настрій чи щось технічне? 🌿",
    "Можеш коротко: 1–2 речення? Я хочу відповісти прямо в точку 🍃",
    "Я тут! 💚🌿 Ти хочеш пораду, відповідь чи просто щоб я послухав?",
]
