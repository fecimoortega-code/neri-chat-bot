...
# Відношення/думки про учасників (ключі в lower)
MEMBER_OPINIONS = {
...
def handle_random_member(q: str) -> str | None:
    # тригери: "випадкового учасника", "random учасника", "випадковий учасник"
...
def handle_member_opinion(raw_text: str, q: str) -> str | None:
    # "що ти думаєш про", "як ти відносишся до", "як ти ставишся до"
    if not (
    ...
    if not name:
        # спроба взяти останнє слово
        parts = q.split()
        name = parts[-1] if parts else ""

    key = normalize_name(name)

    # легкі синоніми/помилки
    if key in ("ритерум",):
        key = "рітерум"
    if key in ("рум",):
        key = "рум"
    if key in ("дейз", "дейзяра"):
        key = key  # є в словнику
    if key in ("ілля", "иля"):
        key = "ілля"
    ...
    # частковий збіг (на випадок кривого написання)
...
def is_cmds_query(q: str) -> bool:
    return (
        ...
        ("повний" in q and "список" in q and "команд" in q) or
        ("дай" in q and "команд" in q) or
        ("свої" in q and "команд" in q) or
        ("назви" in q and "команд" in q)
    )
...
def is_interesting_query(q: str) -> bool:
    return ("розкажи" in q and ("цікав" in q or "цікавеньк" in q))
...
def is_opinion_query(q: str) -> bool:
    return ("як" in q and "вважаєш" in q) or ("що" in q and "думаєш" in q and "взагалі" in q)

def is_support_query(q: str) -> bool:
    return any(x in q for x in ["обій", "будь зі мною", "тривож", "страш", "сумн", "погано", "самот"])
...
async def telegram_webhook(request: Request):
    ...
    if text == "/start":
        ...
        # ===== ПОГОДА =====
            ...
            # 4) що робиш / шо робиш — покривається інтентом, але хай буде
